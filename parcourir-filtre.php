...
//Le type de filtre est récupéré par la valeur de l'input
if($db_found)
{
    if($inThePocket==0)
    { 
        $sql = "SELECT * FROM objet WHERE typeAchat='immediat'";


    } 
    if($inThePocket==1)
    {
        $sql = "SELECT * FROM objet WHERE typeAchat='transaction'";


    }
    if($inThePocket==2)
    {
        $sql = "SELECT * FROM objet WHERE typeAchat='meilleurOffre'";
...
<body>
   ...
   <!-- AFFICHAGE DES BOUTONS DE LA PAGE -->
